...
from ray.autoscaler.aws.config import _subnets_in_network_config, \
    _security_groups_in_network_config, \
    _configure_node_type_from_launch_template, _configure_security_group, \
    _configure_subnet
...
def test_configure_from_launch_template_no_template():
    config = {"head_node": {}}
    expected = {"head_node": {}}
    config = _configure_node_type_from_launch_template(config, "head_node")
...
def test_configure_from_launch_template(ec2_client_stub, key):
    ...
    new_config = _configure_node_type_from_launch_template(config, "head_node")
...
def test_configure_from_launch_template_network_interface_exceptions(
    ...
    with pytest.raises(Exception) as x:
        _ = _configure_node_type_from_launch_template(config, "head_node")
